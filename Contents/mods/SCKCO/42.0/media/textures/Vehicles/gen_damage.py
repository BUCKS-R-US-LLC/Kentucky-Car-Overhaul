...
ALL_ZONE_COLOURS = (GLASS_COLOURS | DOOR_COLOURS | LIGHT_COLOURS | BUMPER_COLOURS
                    | {ROOF_COLOUR, HOOD_COLOUR, TRUNK_COLOUR, BODY_HEAD, BODY_TAIL})


def snap_zone_colours(mask, tol=4):
    """Snap pixels within `tol` (per-channel sum) of a known zone colour to that exact
    colour. Fixes masks whose colours drifted 1-2 values from a resave/compression, which
    would otherwise fail exact-match glass/zone detection. Only remaps near-misses; exact
    pixels and far-off colours (real body paint) are untouched."""
    out = mask.copy()
    zones = np.array(sorted(ALL_ZONE_COLOURS), dtype=np.int16)
    cols, inv = np.unique(mask.reshape(-1, 3), axis=0, return_inverse=True)
    remap = cols.copy()
    for i, c in enumerate(cols):
        if tuple(int(x) for x in c) in ALL_ZONE_COLOURS:
            continue                                  # already exact
        d = np.abs(zones - c.astype(np.int16)).sum(1)
        j = int(d.argmin())
        if d[j] <= tol:
            remap[i] = zones[j]                       # snap near-miss to exact zone colour
    if (remap != cols).any():
        out = remap[inv].reshape(mask.shape).astype(np.uint8)
    return out
...
def gen_scratch(mask, regions, rng, k, hole, intensity=1.0, glass_amp=None):
    H, W = mask.shape[:2]
    ...
    for col, _, is_glass in regions:
        ...
        if is_glass:
            ...
            a = np.clip(a*(glass_amp if glass_amp is not None else GLASS_AMP), 0, 1)   # uniform across windows
            ...
            n_ev = int(np.clip(reg.sum()*intensity/(div*k*k), 2, int(15*max(intensity,1))))
            for _ in range(n_ev):
            ...
            if hit_zone:
                n_hit = int(np.clip(reg.sum()*intensity/(2200*k*k), 3, int(15*max(intensity,1))))
                ...
                n_chip = int(np.clip(reg.sum()*intensity/(2700*k*k), 3, int(13*max(intensity,1))))
...
def gen_blood(mask, targets, rng, k, down, intensity=1.0, splatter_only=False):
    ...
    for col in targets:
        ...
        n_events = int(np.clip(area*intensity/(2400*k*k), 4, int(17*max(intensity,1))))
        ...
        for _ in range(n_events):
            ...
            if splatter_only:                # stage 1: pure spatter dots, no smears/pools
                thr = np.array([rng.normal(0, 1), rng.normal(0, 1)])
                spatter(ox, oy, thr)
                continue
...
def mask_stem(maskpath):
    '''Strip whichever mask suffix the file uses (case-insensitive), return
    (stem, sep_style). sep_style is '' for CamelCase (Vehicle_FooMask) or '_' for
    Vehicle_Foo_mask, inferred from the character before "mask".'''
    name = os.path.basename(maskpath)[:-4]   # drop .png
    low = name.lower()
    if low.endswith('mask'):
        cut = len(name) - 4                  # index where "mask" starts
        if cut > 0 and name[cut - 1] == '_':
            return name[:cut - 1], '_'       # Vehicle_Foo_mask -> sep '_'
        return name[:cut], ''                # Vehicle_FooMask / Vehicle_K5mask -> sep ''
    return name, ''
...
def find_skin(maskpath, stem):
    # overlay outputs must never be mistaken for the vehicle's paint skin, including the
    # numbered stage variants (Damage1/Damage2/Blood1/Blood2) from a previous run.
    known = ('mask', 'damage', 'blood', 'rust', 'lights', 'scratch', 'panelmap',
             'shell', 'overlay', 'burnt', 'wreck')
    cands = []
    for p in sorted(glob.glob(os.path.join(os.path.dirname(maskpath) or '.', stem + '*.png'))):
        ...
        tail_base = tail.rstrip('0123456789')     # drop trailing stage digits
        if tail and not any(tail_base == k or tail_base.endswith(k) for k in known):
            cands.append(p)
...
def process(maskpath, args, cfg=None):
    cfg = cfg or {}
    mask_img = Image.open(maskpath).convert('RGB')
    # optional downscale to a target resolution (e.g. --res 1024) to save game performance.
    # NEAREST keeps the mask's exact zone colours intact (no interpolated edge colours).
    if getattr(args, 'res', None) and mask_img.size[0] != args.res:
        mask_img = mask_img.resize((args.res, args.res), Image.NEAREST)
    mask = np.asarray(mask_img).astype(np.uint8)
    mask = snap_zone_colours(mask)     # fix 1-2 value colour drift from resave/compression
    res = mask.shape[0]
    k = res/REF
    ...
    def alpha_stack(base_rgba, add_rgba):
        """Stack 'add' onto 'base': result contains every mark of base plus add's marks.
        Alpha is the max (superset); RGB is taken from whichever layer is stronger there."""
        b = base_rgba.astype(np.float32); a = add_rgba.astype(np.float32)
        out_a = np.maximum(b[..., 3], a[..., 3])
        # where add is stronger, use its colour; else keep base colour
        use_add = (a[..., 3] >= b[..., 3])[..., None]
        out_rgb = np.where(use_add, a[..., :3], b[..., :3])
        return np.dstack([out_rgb, out_a]).astype(np.uint8)

    line = ''
    if 'damage' in want:
        if args.stages:
            r1 = np.random.default_rng(args.seed)
            sc1, _ = gen_scratch(mask, regions, r1, k, not g_nohole, intensity=1.0, glass_amp=0.55)
            sc1[..., 3] = (sc1[..., 3].astype(np.float32) * 0.75).astype(np.uint8)  # softer stage 1
            r2 = np.random.default_rng(args.seed + 1)
            sc2extra, _ = gen_scratch(mask, regions, r2, k, not g_nohole, intensity=0.6)
            sc2 = alpha_stack(sc1, sc2extra)       # stage 2 = stage 1 + more, guaranteed
            Image.fromarray(sc1, 'RGBA').save(os.path.join(args.out, out_name(stem, sep, SCRATCH_SUFFIX + '1')))
            Image.fromarray(sc2, 'RGBA').save(os.path.join(args.out, out_name(stem, sep, SCRATCH_SUFFIX + '2')))
            line += ' damage: 2 stages (%.3f -> %.3f)' % ((sc1[..., 3] > 8).mean(), (sc2[..., 3] > 8).mean())
        else:
    ...
    if 'blood' in want and not g_noblood:
        ...
        if blood_set:
            ...
            # also exclude hood, doors, trunk by default: the game applies blood there
            # together with damage, so putting it in the overlay too would double up.
            # --blood-hood-doors-trunk re-includes them if ever needed.
            never = GLASS_COLOURS | LIGHT_COLOURS | {ROOF_COLOUR}
            if not getattr(args, 'blood_hdt', False):
                never = never | DOOR_COLOURS | {HOOD_COLOUR, TRUNK_COLOUR}
            tg = [c for c, _, g in regions if not g and tuple(c) not in never]
        tg = [c for c in tg if tuple(c) not in excl_set]
        # blood is a single centralized texture even when damage uses --stages.
        # (--blood-stages re-enables the Blood1/Blood2 split if ever wanted.)
        if args.stages and getattr(args, 'blood_stages', False):
            r1 = np.random.default_rng(args.seed + 100)
            bl1, _ = gen_blood(mask, tg, r1, k, g_down, intensity=1.0, splatter_only=True)
            bl1[..., 3] = (bl1[..., 3].astype(np.float32) * 0.8).astype(np.uint8)  # softer stage 1
            r2 = np.random.default_rng(args.seed + 101)
            bl2extra, _ = gen_blood(mask, tg, r2, k, g_down, intensity=0.6)
            bl2 = alpha_stack(bl1, bl2extra)
            Image.fromarray(bl1, 'RGBA').save(os.path.join(args.out, out_name(stem, sep, BLOOD_SUFFIX + '1')))
            Image.fromarray(bl2, 'RGBA').save(os.path.join(args.out, out_name(stem, sep, BLOOD_SUFFIX + '2')))
            line += '   blood: 2 stages (%.3f -> %.3f)' % ((bl1[..., 3] > 8).mean(), (bl2[..., 3] > 8).mean())
        else:
...
def main():
    ...
    ap.add_argument('--stages', action='store_true',
                    help='emit two stacking stages: Damage1/Damage2, Blood1/Blood2 (stage2 = stage1 + more)')
    ap.add_argument('--res', type=int, default=None,
                    help='downscale mask to NxN before generating (e.g. --res 1024 for 1K overlays)')
    ap.add_argument('--blood-hdt', dest='blood_hdt', action='store_true',
                    help='re-include hood/doors/trunk in blood (off by default: game applies blood there with damage)')
    ap.add_argument('--blood-stages', dest='blood_stages', action='store_true',
                    help='split blood into Blood1/Blood2 too (default: one centralized Blood texture)')
    ...
    if os.path.isdir(a.path):
        # case-insensitive: match any PNG whose name ends in 'mask' (with or without a
        # separator), so Vehicle_K5mask.png, _Mask.png, _mask.png, MASK.png all count.
        all_png = glob.glob(os.path.join(a.path, '**', '*.png'), recursive=True)
        masks = [p for p in all_png
                 if os.path.splitext(os.path.basename(p))[0].lower().endswith('mask')]
        files = sorted(set(masks))
        root = a.path

        # report skipped PNGs that look like textures but weren't recognised as masks,
        # so gaps are visible instead of silent. Overlays and skins are expected non-masks.
        ...
        for p in all_png:
            ...
            if any(t in b for t in overlay_tags):
                continue
            skipped.append(p)
        # subfolders that have PNGs but produced no mask -> likely a naming mismatch
        mask_dirs = {os.path.dirname(p) for p in files}
        png_dirs = {os.path.dirname(p) for p in all_png}
        no_mask_dirs = sorted(d for d in png_dirs - mask_dirs)
        if no_mask_dirs:
            print('WARNING: %d folder(s) have PNGs but no recognised mask '
                  '(check the mask filename ends in "mask"):' % len(no_mask_dirs))
            for d in no_mask_dirs[:40]:
                sample = [os.path.basename(p) for p in all_png
                          if os.path.dirname(p) == d][:3]
                print('   %s   e.g. %s' % (os.path.relpath(d, root), ', '.join(sample)))
            print()
    else:
        files = [a.path]
        root = os.path.dirname(a.path) or '.'
    if not files:
        sys.exit('no mask textures (name must end in "mask") found under %s' % a.path)
    print('found %d mask(s) to process\n' % len(files))
    ...
    cfg_cache = {}
    scan_root = a.path if os.path.isdir(a.path) else (os.path.dirname(a.path) or '.')
    total = len(files)
    failures = []
    for i, f in enumerate(files, 1):
        ...
        if a.out:
            # mirror the source subfolder structure under --out so vehicles don't collide
            rel = os.path.relpath(folder, scan_root)
            out = os.path.join(a.out, rel) if rel != '.' else a.out
        else:
            out = folder                      # write beside each mask (in place)
        os.makedirs(out, exist_ok=True)
        print('[%d/%d] %s' % (i, total, os.path.basename(f)))
        a_run = argparse.Namespace(**{**vars(a), 'out': out})
        # isolate each vehicle: a failure on one must not skip every vehicle after it
        try:
            process(f, a_run, cfg_cache[folder])
        except Exception as e:
            failures.append((f, repr(e)))
            print('   ERROR on %s: %s  (continuing with the rest)'
                  % (os.path.basename(f), e))

    if failures:
        print('\n%d vehicle(s) FAILED and were skipped:' % len(failures))
        for f, err in failures:
            print('   %s\n      %s' % (os.path.relpath(f, scan_root), err))
    else:
        print('\nall %d vehicle(s) processed.' % total)
