...
void main() {
    ...
    col = mix(col, texColorDamage1Shell.xyz * lighting * TintColourNew, texColorDamage1Shell.a * t2en);
    col = mix(col, texColorDamage2Shell.xyz * lighting * TintColourNew, texColorDamage2Shell.a * t3en);

    // Detect window zones (7-12)
    float isWindow = max(
        max(max(texen1[1][2], texen1[1][3]), max(texen1[2][0], texen1[2][1])),
        max(texen1[2][2], texen1[2][3])
    );

    // Uninstalled parts: discard entirely so depth buffer is clean
    if (t4en > 0.5) {
        discard;
    }

    // Window zones: checkerboard dither for transparency without
    // writing semi-transparent depth that occludes doors/player
    if (isWindow > 0.5) {
        ...
        // Tint surviving pixels slightly blue-grey for a glass look
        col = mix(col, vec3(0.7, 0.75, 0.8), 0.25);
        gl_FragColor = vec4(col, 1.0);
    } else {
        // Solid parts: discard only if base texture is transparent
        if (tex.a < 0.05) {
            discard;
        }
