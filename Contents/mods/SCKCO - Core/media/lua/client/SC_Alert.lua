...
function donationSystem:prerender()
    ...
    self:drawText("Kentucky Car Overhaul", headerX, headerY, 1, 1, 1, 0.9, donationSystem.headerFont)

    local bodyX, bodyY = self.padding*1.7, (self.padding*1.4)+donationSystem.headerHeight
    self:drawText("We have decided to push armor and the rest of the vehicles of 90's into one update. Expect the update in Build 42.", bodyX, bodyY, 1, 1, 1, 0.8, donationSystem.bodyFont)
    self:drawText("The update will drop amongst the disruption of Build 42, as per the developers warnings. Uninstall if you do not consent.", bodyX, bodyY, 1, 1, 1, 0.8, donationSystem.bodyFont)
...
function donationSystem:onClickDonate() openUrl("https://github.com/BUCKS-R-US-LLC/Kentucky-Car-Overhaul/issues") end
...
function donationSystem:initialise()
    ...
    self.donate = ISButton:new(((self.width-btnWid)/2)-self.padding*1.3, self:getHeight()-(self.padding*1.3)-btnHgt, btnWid, btnHgt, "Report A Bug", self, donationSystem.onClickDonate)
